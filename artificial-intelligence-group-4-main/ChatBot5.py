...
# Prints to the 'server.js' file as a 'pgQuery' object, which is used to perform a PostgreSQL (database) query
def get_psql_query(table, params):
    message_dict = {
        "type": "pgQuery",
        "content": {
            "table": table,
...
class TravelBot(KnowledgeEngine):
    ...
    @Rule(Ticket(departure=~L(None), arrival=~L(None), time=~L(None), isleavingtime=W(),
                 ...
                 returnEndLocation=~L(None), returnTime=None, returnIsLeavingTime=W(), returnDay=W(),
                 returnMonth=W()))
    def ask_for_a_return_time(self):
        responses = [
            "I've got both destination for you return ticket but i don't know the time you'd like to travel,"
    ...
    def got_all_delay_info(self):
        ...
        print_bot_message(self.delay_info)
